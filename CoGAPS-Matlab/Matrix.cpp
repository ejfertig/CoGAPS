...
// set a row of the matrix to passed array (assumes same dimensions)
// implemented to fix rows at a time with maps
void Matrix::setRow(const double *theRow, int RowNum){
        for (int n=0; n<_n_col;++n) {        
	  _Matrix[RowNum][n] = theRow[n]; 
        }
    }
	
// set a column of the matrix to passed array (assumes same dimensions)
// implemented to fix columns at a time with maps (?? necessary ??)
void Matrix::setCol(const double *theCol, int ColNum){
        for (int n=0; n<_n_row;++n) {        
	  _Matrix[n][ColNum] = theCol[n]; 
        }
    } 	
...
double * Matrix::get_Row(int rowNum) const {
