...
namespace gaps {
  ...
  double AtomicSupport::getNAtomPriorProb(int delAtom, bool log) {
    ...
    if (_nAtom + delAtom < 0 || _nAtom + delAtom > _NatomLength) {
      if (log) {
	return DOUB_NEGINF;
      }
      return 0.;
  ...
  void AtomicSupport::writeAtomicDiagnostics() {
	...
	vector< unsigned int> nAtomPerBin(_nBin);
    //double massPerBin[_nBin];
	vector<double> massPerBin(_nBin);
    for (iBin = 0; iBin < _nBin; iBin++) {
      nAtomPerBin[iBin] = 0;
      massPerBin[iBin] = 0.;
    }
  ...
  void AtomicSupport::updateAtomicBins(vector<double> binProbabilities, 
				       unsigned int length,
	...
	vector<unsigned long long> origBoundaries(_nBin);
	vector<unsigned long long> newBoundaries(_nBin);
	...
	//Change here, just make a vector into a vector)
     // vector<double> binProbVector (binProbabilities, binProbabilities+length);
	 vector<double>binProbVector;
	 binProbVector = binProbabilities;
    ...
    if (_AtomicDomain.size() > 0) {
	  ...
	  vector<unsigned long long> newLocation(_AtomicDomain.size());
      //double atomSize[_AtomicDomain.size()];
	  vector<double> atomSize(_AtomicDomain.size());
