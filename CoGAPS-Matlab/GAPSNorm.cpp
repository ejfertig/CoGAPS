...
{  
  ...
  //                          / S_{mj}^2
    ...
    // Extract where and how large the change is
        ...
        switch (matrix_label){
	      ...
	      //double M[1][nCol];
		  //DYNAMIC ARRAYS CHANGED TO VECTORS TO WORK WITH WINDOWS SDK AND MATLAB
		  vector< vector <double> > M(1, vector<double>(nCol));
	      for (unsigned int iCol=0; iCol < nCol; ++ iCol){
		...
		for (unsigned int iPattern=0; iPattern < nFactor; ++iPattern){
	      ...
	      //double M[nRow][1];
		  vector< vector <double> > M(nRow, vector<double>(1));
	      for (unsigned int iRow=0; iRow < nRow; ++ iRow){
  ...
  double GAPSNorm::calcDeltaLL2E(char matrix_label,
    ...
    switch (matrix_label){
	  ...
	  // and M[chRow[1]][].
	  //double M[2][nCol];
	  vector< vector <double> > M(2, vector<double>(nCol));
	  ...
	  // and M[][chCol[1]].
	  //double M[nRow][2];
	  vector< vector <double> > M(nRow, vector<double>(2));
  ...
  // delloglikelihood = \sum_{ij} [2*M_{ij}*(delA*P)_{ij}-(delA*P)_{ij}^2] /2/S_{ij}^2
    ...
    // ---- Form M = D - A*P -----
    //double M[nRow][nCol];
	vector< vector <double> > M(nRow, vector<double>(nCol));
    for (unsigned int iRow=0; iRow < nRow; ++iRow){
        for (unsigned int iCol=0; iCol < nCol; ++ iCol){
    ...
    switch(matrix_label){
        case 'A':{
			...
			vector< vector <double> > delA(nRow, vector<double>(nFactor));
			...
			vector< vector <double> > delAP(nRow, vector<double>(nCol));
            for (unsigned int iRow=0; iRow < nRow; ++iRow){
                for (unsigned int iCol=0; iCol < nCol; ++iCol){
		  delAP[iRow][iCol] = 0.;
                    for (unsigned int iPattern=0; iPattern < nFactor; ++iPattern){
        ...
        case 'P':{
			...
			vector< vector <double> > delP(nFactor, vector<double>(nCol));
			...
			vector< vector <double> > AdelP(nRow, vector<double>(nCol));
            for (unsigned int iRow=0; iRow < nRow; ++iRow){
                for (unsigned int iCol=0; iCol < nCol; ++iCol){
		  AdelP[iRow][iCol]=0.;
                    for (unsigned int iPattern=0; iPattern < nFactor; ++iPattern){
...
// ---------------------------------------------------------------------------
  // Calcuation of the change in the log-likelihood when matrix A (or P) is augmented 
  // with a change that occurs in a fixed pattern. This means that an entire row of 
  // A or P is changed. 
  // Mathematically, it calculates changes to the following:
  // 
  // Let the only change be delA, define M = D-AP;
  // delloglikelihood = \sum_{ij} [2*M_{ij}*(delA*P)_{ij}-(delA*P)_{ij}^2] /2/S_{ij}^2
    ...
    double GAPSNorm::calcDeltaLLMap(char matrix_label,
				double const * const * D, double const * const * S, 
				double const * const * A, double const * const * P, 
				double * newRow, unsigned int chRow, unsigned int nRow, 
				unsigned int nCol, unsigned int nFactor) {
    ...
    switch(matrix_label){
        case 'A':{
			...
			// The change here is along one row, chRow:
            for (unsigned int iCol=0; iCol < nFactor; ++iCol){
                delA[chRow][iCol] += newRow[iCol];
