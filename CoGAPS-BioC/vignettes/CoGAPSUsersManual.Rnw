...
\par On some platforms, the dynamic libraries may not be properly linked for loading the CoGAPS package, leading to an error message such as
\begin{verbatim}
...
\par Installation of CoGAPS on Windows through compiling requires the installation of Rtools and use of the cygwin environment.  Please contact Elana J. Fertig \url{ejfertig@jhmi.edu} or Michael F. Ochs \url{ochsm@tcnj.edu} for installation instructions.  However, installation directly from Bioconductor avoiding local compilation should be straightforward.
...
\item[simulation_id]{The base name to attach to files with atoms if created.}
...
\par In this example, we have simulated data in EasySimGS (DGS) with three known patterns (PGS) and corresponding amplitude (AGS) with specified activity in two gene sets (gs).  In this data set, each gene set is overexpressed in one the simulated patterns and underexpressed in one.
...
Figure \ref{fig:GS} shows the results from running CoGAPS on the SimpleSim data.  
%In addition, 
%gene set statistics can be calculated on the results of GAPS by using the \texttt{CalcCoGAPSStat}
%method.  The needed inputs are the mean and standard deviations for the $\mathbf{A}$ matrix
%provide in the output of GAPS as \textt{results$Amean} and \textt{results$Asd} together with
%the gene sets as a list or data.frame. 

%<<label=GAPSSimpSim, echo=TRUE>>=
%gsRes <- calcCoGAPSStat(results$Amean, results$Asd, GSets)
%gsRes$GSActEst
%@ 
%
%The activity estimates are printed to screen here since in this simple simulation there are only
%two sets and three patterns.  In general, they are analyzed in the returned list either looking
%for significant upregulated sets in the \texttt{GSUpreg} object, significantly underrepresented
%sets in the \texttt{GSUpreg} object, or activity estimates derived from these in
%the \texttt{GSActEst} object (see \cite{Ochs2009} for details on calculations).
...
the statisitcal analysis of \textt{CalcCoGAPSStat}.  Specifically, CoGAPS computes a $Z$-score based statistic on each column of the ${\bf{A}}$ matrix developed in \cite{Ochs2009}.  The resulting $Z$-score for pattern $p$ and gene set $i$, $\mathcal{G}_{i}$, with $G$ elements is given by
\begin{equation}
...
data('SimpSim')
nIter <- 5000 
results <- CoGAPS(SimpSim.D, SimpSim.S, GSets,
                  numPatterns=3,
                  nEquil=nIter, nSample=nIter
                  plot=FALSE)
